...
export class ProductosComponent {
  ...
  exportarCatalogoPDF() {
    ...
    const cardWidth = 60;   // Ancho de cada card
    const cardHeight = 80;  // Alto de cada card
    const margin = 10;      // Margen entre cards
    const startX = 10;      // Margen lateral
    ...
    productosOrdenados.forEach((producto, index) => {
      ...
      if (producto.IMAGE_PATH) {
        ...
        // 👇 MUY IMPORTANTE: cargar imágenes sincrónicas con jsPDF requiere callback
        // así que hacemos un "addImage" con try/catch
        try {
          doc.addImage(img, "JPEG", x + 5, y + 5, cardWidth - 10, 35);
        } catch {
      ...
      doc.text(
        ...
        x + cardWidth / 2,
        y + 45,
        { align: "center", maxWidth: cardWidth - 8 }
      );
      ...
      // Posicionar la siguiente card
      x += cardWidth + margin;

      // Si se sale del ancho de la página, salto de línea
      if (x + cardWidth > pageWidth - startX) {
        x = startX;
        y += cardHeight + margin;
      }

      // Si se pasa del alto de la hoja, nueva página
      if (y + cardHeight > pageHeight - startY) {
        doc.addPage();
        x = startX;
        y = startY;
      }
