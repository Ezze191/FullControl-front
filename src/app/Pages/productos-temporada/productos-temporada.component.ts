import { AgregarProductoTemporadaComponent } from './agregar-producto-temporada/agregar-producto-temporada.component';
...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, ReactiveFormsModule, HttpClientModule, FormsModule, AgregarProductoTemporadaComponent],
    templateUrl: './productos-temporada.component.html',
    styleUrl: './productos-temporada.component.css'
})
export class ProductosTemporadaComponent {
    ...
    proveedores: string[] = [];
    temporadas: string[] = []; // Nueva lista para filtro

    // Filtros
    filtroProveedor: string = '';
    filtroTemporada: string = ''; // Nuevo filtro
    ...
    constructor(
        private productoService: ProductosTemporadaService,
    ...
    obtenerProductos() {
        ...
        this.productoService.getProductos().subscribe(data => {
            ...
            this.temporadas = [...new Set(data.map(p => String(p.TEMPORADA)))].filter(p => p).sort();
    ...
    //modalEditarAbierto
    modalEdit(producto: ProductoTemporada) {
    ...
    get productosFiltrados() {
        return this.productos.filter(p => {
            ...
            return matchesSearch && matchesProvider && matchesTemporada &&
    ...
    exportarPDF() {
        ...
        doc.setFontSize(16);
        doc.setTextColor(40);
        doc.text('Reporte de Productos de Temporada', 14, 15);

        const fechaHora = new Date().toLocaleString();
        doc.setFontSize(11);
        doc.text(`LISTA DE PRODUCTOS - ${fechaHora}`, 14, 22);

        const columnas = ['TEMPORADA', 'NOMBRE', 'PLU', 'EXISTENCIA', 'PRECIO COMPRA', 'PRECIO VENTA', 'GANANCIA', 'PROVEEDOR', 'ÚLTIMO INGRESO'];

        const filas = this.productosFiltrados.map(producto => [
            String(producto.TEMPORADA),
        ...
        autoTable(doc, {
            ...
            headStyles: {
                fillColor: [255, 193, 7], // Warning color (Yellow/Orange)
                textColor: 0,
                halign: 'center'
            },
            bodyStyles: {
        ...
        const now = new Date();
        const fileName = `reporte-productos-temporada-${now.toISOString().slice(0, 10)}-${now.getHours()}-${now.getMinutes()}.pdf`;
        doc.save(fileName);
    }
    ...
    exportarExcel() {
        ...
        const now = new Date();
        const fileName = `reporte-productos-temporada-${now.toISOString().slice(0, 10)}-${now.getHours()}-${now.getMinutes()}.xlsx`;
    ...
    async exportarCatalogo() {
        ...
        doc.setFontSize(22);
        doc.setFont('helvetica', 'bold');
        const tituloTexto = 'CATÁLOGO DE TEMPORADA';
        ...
        for (let i = 0; i < productos.length; i++) {
            ...
            if (necesitaNuevaPagina) {
                ...
                x = margin;
                primeraPagina = false;
                letraActualPagina = ''; // Resetear para nueva página
            }
            ...
            // Header de la card (color warning)
        ...
        // Guardar el PDF
        const now = new Date();
        const fileName = `catalogo-temporada-${now.toISOString().slice(0, 10)}.pdf`;
        doc.save(fileName);
        this.SpinnerLoading = false;
    }
